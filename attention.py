...
class TransformerBlock(nn.Module):
    def __init__(self, k, heads):
        super().__init__()
        self.attention_mask = torch.LongTensor([[1, 1, 1], [1, 1, 0]])
        self.attention = SelfAttention(k, num_attention_heads=heads, dropout_prob=0.2)
...
low_level_feature = torch.Tensor(2, 48, 128, 128)  # 生成2*3的张量

# low_level_feature = self.attentionblock_low(low_level_feature) # [2,48,128,128]
# output_feature =
# output_feature = self.attentionblock_outfeature(output_feature) # [2,256,32,32]

# cat_feature = [2,304,128,128]  classifier_output[2,21,128,128]
mask=torch.Tensor(2, 48, 128, 128)
model = TransformerBlock(256, 8)
x=model(low_level_feature,mask)
print(x)
#print(model)
